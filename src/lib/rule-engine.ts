...
export function evaluateDeterministicRules(
  ...
  const safeDomain = crawl.finalUrl && crawl.finalUrl.startsWith('http') ? new URL(crawl.finalUrl).hostname.replace(/^www\./, "") : "example.com";
    ...
    codeSnippet: `<script type="application/ld+json">
{
  ...
  "@type": "LocalBusiness",
  "name": "${name}",
  "url": "${crawl.finalUrl || input.url || "https://example.com"}",
  "telephone": "${crawl.contactInfo.phones[0] || "+1-000-000-0000"}",
  "email": "${crawl.contactInfo.emails[0] || "contact@" + safeDomain}",
  "address": {
    "@type": "PostalAddress",
